...
<html>
	<head>
		...
		<!-- 
			...
			-기본 데이터 전송방식
			-전송 데이터가 주소에 노출
			-보안성 취약하고 POST 대비 빠른 전송
			
			POST 방식
			- 전송 데이터를 요청 객체 body에 기록되어 전송되는 방식
			- 보안에 유리하고 대량데이터 전송에 적합
	...
	<body>
		...
		<form action="./proc/resultPost.jsp" method="post">
			<table border = "1">
				...
				<tr>
					<td>성별</td>
					<td><label><input type="radio" name="gender" value="M">남</label></td>
					<td><label><input type="radio" name="gender" value="F">여</label></td>
				</tr>
				<tr>
					<td>주소</td>
					<td><select name="addr">
		...
		<form action="./proc/resultPost.jsp" method="post">
			...
			헤더정보#2 : <%=request.getHeader("cookie") %><br>
